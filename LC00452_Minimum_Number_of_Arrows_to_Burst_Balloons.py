# LC00452_Minimum_Number_of_Arrows_to_Burst_Balloons.py
...
# Idea: sort based on end location, then always shoot at the earliest end, if a balloon we are visiting now is already shot by last time, 
# ignore it. 

# from functools import cmp_to_key

# def compare_balloon(x,y):
#     return x[1]-y[1]

class Solution:
    def findMinArrowShots(self, points: List[List[int]]) -> int:
        #points.sort(key=cmp_to_key(compare_balloon))
        points.sort(key=lambda p: p[1])

        last_shoot_pos=points[0][1]
        count_shoot=1
        for i in range(1,len(points)):
            if points[i][0]<=last_shoot_pos:
                continue
            else:
                last_shoot_pos=points[i][1]
                count_shoot+=1
        return count_shoot
