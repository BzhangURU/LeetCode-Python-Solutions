...
# Following the order of appearance in order, "b", "c", and "a" from s should be arranged as "b", "c", "a". "d" c
# an be placed at any position since it's not in order. The output "bcad" correctly follows this rule. 
...
# Idea: use two dict to save chars in s. one is for chars in order, one is not.

class Solution:
    def customSortString(self, order: str, s: str) -> str:
        order_list=list(order)
        set_order=set(order_list)
        ...
        for i in range(len(s)):
            if s[i] in set_order:
                if s[i] in dict_s_in_order:
                    dict_s_in_order[s[i]]+=1
                else:
                    dict_s_in_order[s[i]]=1
            else:
                if s[i] in dict_s_not_in_order:
                    dict_s_not_in_order[s[i]]+=1
                else:
                    dict_s_not_in_order[s[i]]=1

        output_list=[]
        for i in range(len(order)):
            if order[i] in dict_s_in_order:
                iter_total=dict_s_in_order[order[i]]
                for j in range(iter_total):
                    output_list.append(order[i])
        for k,v in dict_s_not_in_order.items():
            for j in range(v):
