...
#Idea: 
# DP
#For example, if poured=5, then first we have
#  5
# 0 0
#0 0 0
...
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        prev_row=[float(poured)]
        if query_row==0:
            return min(1.0, prev_row[0])
        for row in range(query_row):
            cur_row=[0.0 for i in range(len(prev_row)+1)]
            for ind in range(len(cur_row)):
                if ind==0:
                    cur_row[0]=max(0.0,(prev_row[0]-1.0)/2)
                elif ind==len(cur_row)-1:
                    cur_row[-1]=max(0.0,(prev_row[-1]-1.0)/2)
                else:
                    cur_row[ind]=max(0.0,(prev_row[ind-1]-1.0)/2)+max(0.0,(prev_row[ind]-1.0)/2)
            prev_row=cur_row.copy()
        output=min(1.0,cur_row[query_glass])
        return output
