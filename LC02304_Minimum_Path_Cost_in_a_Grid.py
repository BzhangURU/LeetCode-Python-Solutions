# LC02304_Minimum_Path_Cost_in_a_Grid.py
...
# such that x < m - 1, you can move to any of the cells (x + 1, 0), (x + 1, 1), ..., (x + 1, n - 1). Note that 
# it is not possible to move from cells in the last row.
...
class Solution:
    def minPathCost(self, grid: List[List[int]], moveCost: List[List[int]]) -> int:
        m=len(grid)
        n=len(grid[0])
        cost_matrix=[[-1 for _ in range(n)] for _ in range(m)]
        
        for col in range(n):
            cost_matrix[0][col]=grid[0][col]

        for cur_row in range(1,m):
            for cur_col in range(n):
                for prev_col in range(n):
                    if cost_matrix[cur_row][cur_col]==-1 or \
                        cost_matrix[cur_row-1][prev_col]+moveCost[grid[cur_row-1][prev_col]][cur_col]+grid[cur_row][cur_col]<cost_matrix[cur_row][cur_col]:
                        cost_matrix[cur_row][cur_col]=cost_matrix[cur_row-1][prev_col]+moveCost[grid[cur_row-1][prev_col]][cur_col]+grid[cur_row][cur_col]

        return min(cost_matrix[m-1])
