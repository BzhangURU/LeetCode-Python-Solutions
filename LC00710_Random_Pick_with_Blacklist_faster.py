...
# 4. based on 3, only generate a random float 0-1. 
import random
class Solution:
    def __init__(self, n: int, blacklist: List[int]):
        blacklist.sort()
        self.total_available_numbers=float(n-len(blacklist))
        ...
        for i in range(len(blacklist)):
            if blacklist[i]!=start_num:
                frac_start=float(accumulated_numbers)/self.total_available_numbers #accumulated_fraction
                frac_end=frac_start+float(blacklist[i]-start_num)/self.total_available_numbers
        ...
        if len(blacklist)==0 or blacklist[-1]<n-1:
            frac_start=float(accumulated_numbers)/self.total_available_numbers#accumulated_fraction
            frac_end=1.0#frac_start+float(n-start_num)/self.total_available_numbers
    ...
    def pick(self) -> int:
        ...
        random_num=int((random01-self.chunks[chunk_ind][0])/(1/self.total_available_numbers))
