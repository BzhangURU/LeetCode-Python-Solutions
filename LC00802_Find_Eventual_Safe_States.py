# LC00802_Find_Eventual_Safe_States.py

# There is a directed graph of n nodes with each node labeled from 0 to n - 1. 
# The graph is represented by a 0-indexed 2D integer array graph where graph[i] is 
# an integer array of nodes adjacent to node i, meaning there is an edge from node i 
...
#Idea: The output nodes should not be inside any loop or finally go into a loop.
# For each not confirmed node, use depth first search, stop if we return back to visited nodes, 
# then put all visited nodes to "confirmed dangerous node". All other nodes will be safe nodes.

from typing import List

def DFS(start_node, graph, set_visited_nodes, set_visited_nodes_without_remove, list_confirmed_danger_nodes):
    set_visited_nodes_without_remove.add(start_node)
    if start_node in set_visited_nodes or list_confirmed_danger_nodes[start_node]==1:
        for i in set_visited_nodes:
            list_confirmed_danger_nodes[i]=1

        for i in set_visited_nodes_without_remove:
            if i not in set_visited_nodes:
                list_confirmed_danger_nodes[i]=0
        #Maybe we are visiting a start_node that is list_confirmed_danger_nodes[start_node]==1 but not in set_visited_nodes
        list_confirmed_danger_nodes[start_node]=1
        return False
    set_visited_nodes.add(start_node)
    for i in range(len(graph[start_node])):
        #The sentence "start_node not in set_visited_nodes_without_remove" is crucial to get linear time complexity. imagine a graph where node 0 points to 1, 1->2, 2->3, ..., 100->101, then all node 0-101 point to
        # node 102, and node 102 has a long chain, then that long chain would be visited many times.  
        #Another situation is: b1->b1, b1->a, ..., bn->bn, bn->a. But if a's index is smaller, then we would visit a first, and set a as confirmed safe.
        # if b is smaller, then we would detect loop first before visiting a.  
        if list_confirmed_danger_nodes[graph[start_node][i]]!=0 and (graph[start_node][i] in set_visited_nodes or graph[start_node][i] not in set_visited_nodes_without_remove):
            output=DFS(graph[start_node][i],graph,set_visited_nodes,set_visited_nodes_without_remove,list_confirmed_danger_nodes)
            if output==False:
                return False
        
    set_visited_nodes.remove(start_node)
...
class Solution:
    def eventualSafeNodes(self, graph: List[List[int]]) -> List[int]:
        ...
        list_confirmed_danger_nodes=[-1]*len(graph)#initialized: -1, confirmed danger: 1, confirmed safe: 0
        set_visited_nodes=set()
        set_visited_nodes_without_remove=set()
        for node in range(len(graph)):
            if list_confirmed_danger_nodes[node]!=-1:
                continue
            set_visited_nodes.clear()
            set_visited_nodes_without_remove.clear()
            output=DFS(node,graph,set_visited_nodes,set_visited_nodes_without_remove,list_confirmed_danger_nodes)
    
            if output:
                for safe_node in set_visited_nodes_without_remove:
                    list_confirmed_danger_nodes[safe_node]=0
                    
            
        result=[]
        for i in range(len(graph)):
            if list_confirmed_danger_nodes[i]!=1:
                result.append(i)
...
print(my_solu.eventualSafeNodes(graph))
