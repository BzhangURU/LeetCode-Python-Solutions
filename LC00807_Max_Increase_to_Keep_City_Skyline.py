# LC00807_Max_Increase_to_Keep_City_Skyline.py

# There is a city composed of n x n blocks, where each block contains a single building shaped like a vertical square prism. 
# You are given a 0-indexed n x n integer matrix grid where grid[r][c] represents the height of the building located in 
# the block at row r and column c.
...
#Idea: first calculate max for each row and each col, then increase to min of the two max. 

class Solution:
    def maxIncreaseKeepingSkyline(self, grid: List[List[int]]) -> int:
        rows=len(grid)
        cols=len(grid[0])
        max_each_row=[0 for i in range(rows)]
        max_each_col=[0 for i in range(cols)]
        for i in range(rows):
            max_each_row[i]=max(grid[i])
        for i in range(cols):
            max_each_col[i]=max([grid[j][i] for j in range(rows)])
        result=0
        for row in range(rows):
            for col in range(cols):
                max_height=min(max_each_row[row],max_each_col[col])
                if max_height>grid[row][col]:
                    result+=max_height-grid[row][col]
        return result
