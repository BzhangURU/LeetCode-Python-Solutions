...
#                                                                    pixel, either horizontally or vertically) 
...
# From the center of the image with position (sr, sc) = (1, 1) (i.e., the red pixel), all pixels connected by 
# a path of the same color as the starting pixel (i.e., the blue pixels) are colored with the new color.

# Note the bottom corner is not colored 2, because it is not horizontally or vertically connected to the 
# starting pixel.
...
# The starting pixel is already colored with 0, which is the same as the target color. Therefore, 
...
class Solution:
    def floodFill(self, image: List[List[int]], sr: int, sc: int, color: int) -> List[List[int]]:
        ...
        my_set=set()
        my_q=deque([[sr,sc]])
        my_set.add(sr*100+sc)

        while len(my_q)>0:
            row, col=my_q.popleft()

            if row>0:
                if image[row-1][col]==image[row][col] and (row-1)*100+col not in my_set:
                    my_set.add((row-1)*100+col)
                    my_q.append([row-1, col])
            if col>0:
                if image[row][col-1]==image[row][col] and (row)*100+col-1 not in my_set:
                    my_set.add((row)*100+col-1)
                    my_q.append([row, col-1])
            if row<len(image)-1:
                if image[row+1][col]==image[row][col] and (row+1)*100+col not in my_set:
                    my_set.add((row+1)*100+col)
                    my_q.append([row+1, col])
