# LC00430_Flatten_a_Multilevel_Doubly_Linked_List.py
...
class Solution:
    def my_flatten(self, head):#return both head and tail
        node=head
        tail=head.next
        ...
        while node:
            ...
            if node.child is None:
                ...
                node_next=node.next
                child_head,child_tail=self.my_flatten(node.child)
    ...
    def flatten(self, head: 'Optional[Node]') -> 'Optional[Node]':
        if head is None:
            return head
        head, tail = self.my_flatten(head)
        return head
